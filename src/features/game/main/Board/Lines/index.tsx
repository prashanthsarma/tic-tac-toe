...
export const Lines: React.FC = () => {
  ...
  for (let i = 1; i < BoardRows; i++) {
    ...
    lines.push(<svg key={`rowLine${i}`} viewBox={`0 0 ${width} 2`} style={{ top: `${i * CellWidth}px`, position: "absolute", left: "0px" }}>
  ...
  for (let i = 1; i < BoardColumns; i++) {
    ...
    lines.push(<svg key={`colLine${i}`} viewBox={`0 0 2 ${height}`} style={{ left: `${i * CellWidth}px`, position: "absolute", top: "0px", height }}>
